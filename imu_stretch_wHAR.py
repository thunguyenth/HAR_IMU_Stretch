""" *************************************** IMUs + STRETCH SENSORS - wHAR dataset **************************************
 - This is a Deep learning based HAR models using IMUs and Stretch sensors
...
def load_sensor_fusion_data(path='data/w-har_data/data_wHAR'):
    """
...
def evaluate_model(model, _X_train, _y_train, _X_val, _y_val, _X_test, _y_test):
    config = Config()
    rlrop = C.ReduceLROnPlateau(monitor='loss', factor=1/config.RLROP, patience=10)
...
class Config:
    ...
    LR = 0.001
    VERBOSE = 2
    RLROP = 5  # reduction rate of reducing learning rate on plateau


if __name__ == '__main__':

    DATA_PATH = 'data/w-har_data/data_wHAR_2021120101'
    WIN_SIZE = 50
    N_HIDDENS = 128
    CLASS_NAMES = utils.labels_wHAR
    COMBINE_OPTIONS = ['imu', 'stretch', 'imu_stretch']
    MODEL_NAMES = ['LSTM', 'CNN_1D', 'CNN1D_LSTM_v2']

    # Change the following variables with the above COMBINE_OPTIONS and MODEL_NAMES
    combine_option = 'imu_stretch'
    model_name = "CNN1D_LSTM_v2"
    ...
    savemat(f"results_wHAR_20211201/{model_name}-{combine_option}-{dt.date()}_{dt.hour}_{dt.minute}.mat", mat_file)

    # # Using only once to save the y_test to mat file
    # y_test_mat_file = {'y_test': y_test2}
    # savemat('data/data_wHAR_2021120101_y_test.mat', y_test_mat_file)
